...
class ValidatorFactory {
	...
	public static BeanAnalyser getValidator(Class<?> type, Field field, Object o, Annotation annotation){
		...
		if(NotNull.class.getSimpleName().equals(name)){
			...
			NotNullValidatorType notNullValidatorType = new NotNullValidatorType(type, annotation, field, o, exception, messageException);
			if(simpleAnnotation.emptyIsNull()){
				notNullValidatorType.setBeanAnalyser( new NotEmptyValidator(type, annotation, field, o, exception, messageException));
			}
		...
		if(NonSpecialChar.class.getSimpleName().equals(name)){
			...
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new NonSpecialCharValidator(type, annotation, field, o, exception, messageException);
		} 
		if(NotEmpty.class.getSimpleName().equals(name)){
			NotEmpty simpleAnnotation = (NotEmpty) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new NotEmptyValidator(type, annotation, field, o, exception, messageException);
		...
		if(Range.class.getSimpleName().equals(name)){
			...
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new RangeValidator(type, annotation, field, o, exception, messageException);
		} 
		if(Size.class.getSimpleName().equals(name)){
			Size simpleAnnotation = (Size) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new SizeValidator(type, annotation, field, o, exception, messageException);
		} 
		...
		if(RollBack.class.getSimpleName().equals(name)){
			...
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new RollBackValidator(type, annotation, field, o, exception, messageException);
		} 
		if(Trigger.class.getSimpleName().equals(name)){
			Trigger simpleAnnotation = (Trigger) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new TriggerValidator(type, annotation, field, o, exception, messageException);
		} 
		...
		if(CNPJ.class.getSimpleName().equals(name)){
			...
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new CNPJValidator(type, annotation, field, o, exception, messageException);
		} 
		...
		if(CPF.class.getSimpleName().equals(name)){
			...
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new CPFValidator(type, annotation, field, o, exception, messageException);
		} 
		if(Decimal.class.getSimpleName().equals(name)){
			Decimal simpleAnnotation = (Decimal) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new DecimalValidator(type, annotation, field, o, exception, messageException);
		} 
		if(Email.class.getSimpleName().equals(name)){
			Email simpleAnnotation = (Email) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new EmailValidator(type, annotation, field, o, exception, messageException);
		} 
		if(InscricaoEstadual.class.getSimpleName().equals(name)){
			InscricaoEstadual simpleAnnotation = (InscricaoEstadual) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new InscricaoEstadualValidator(type, annotation, field, o, exception, messageException);
		} 
		if(Int.class.getSimpleName().equals(name)){
			Int simpleAnnotation = (Int) annotation;
			exception = simpleAnnotation.exception();
			messageException = simpleAnnotation.messageException();
			return new IntValidator(type, annotation, field, o, exception, messageException);
		} 
